...
ggplot(df, aes(x = Imputation_method, y = ARI, fill = Imputation_method)) +
  ...
  labs(title = "Adjusted Rand Index (ARI) Comparison", x = "Imputation Method", y = "ARI") +
...
ggplot(df, aes(x = Imputation_method, y = NMI, fill = Imputation_method)) +
  geom_boxplot() +
  facet_grid(~Missingness_level) +
  labs(title = "Normalized Mutual Information (NMI) Comparison", x = "Imputation Method", y = "NMI") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
ggplot(df, aes(x = Imputation_method, y = Silhouette_Score, fill = Imputation_method)) +
  geom_boxplot() +
  facet_grid(~Missingness_level) +
  labs(title = "Silhouette Score Comparison", x = "Imputation Method", y = "Silhouette Score") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
# Conclusion
The visual analytics of our simulation study have elucidated the differential impacts of various imputation methods on the performance of K-means clustering in the context of missing data. The Adjusted Rand Index (ARI) and Silhouette Score comparisons across missingness levels (5%, 10%, 20%, 40%) under three missing data mechanisms (MAR, MCAR, MNAR) reveal distinct patterns. The KNN imputation method generally maintains higher ARI and Silhouette Scores compared to mean and median imputation, suggesting its effectiveness in preserving the original data structure despite the presence of missing values.