...
for (missing_rate in missingness_levels) {
  for (i in 1:repetitions) {
    ...
    performance_metrics_list[['mean_MCAR']] <- c("MCAR", "Mean", calculate_performance_metrics(imputed_data_mean_MCAR, true_labels, kmeans_result_mean_MCAR$cluster))
    performance_metrics_list[['median_MCAR']] <- c("MCAR", "Median", calculate_performance_metrics(imputed_data_mean_MCAR, true_labels, kmeans_result_median_MCAR$cluster))
    performance_metrics_list[['knn_MCAR']] <- c("MCAR", "Knn", calculate_performance_metrics(imputed_data_mean_MCAR, true_labels, kmeans_result_knn_MCAR$cluster))
    
    performance_metrics_list[['mean_MNAR']] <- c("MNAR", "Mean", calculate_performance_metrics(imputed_data_mean_MNAR, true_labels, kmeans_result_mean_MNAR$cluster))
    performance_metrics_list[['median_MNAR']] <- c("MNAR", "Median", calculate_performance_metrics(imputed_data_median_MNAR, true_labels, kmeans_result_median_MNAR$cluster))
    performance_metrics_list[['knn_MNAR']] <- c("MNAR", "Knn", calculate_performance_metrics(imputed_data_knn_MNAR, true_labels, kmeans_result_knn_MNAR$cluster))
    
    performance_metrics_list[['mean_MAR']] <- c("MAR", "Mean", calculate_performance_metrics(imputed_data_mean_MAR, true_labels, kmeans_result_mean_MAR$cluster))
    performance_metrics_list[['median_MAR']] <- c("MAR", "Median", calculate_performance_metrics(imputed_data_median_MAR, true_labels, kmeans_result_median_MAR$cluster))
    performance_metrics_list[['knn_MAR']] <- c("MAR", "Knn", calculate_performance_metrics(imputed_data_knn_MAR, true_labels, kmeans_result_knn_MAR$cluster))

metrics_df_row <- as.data.frame(t(unlist(performance_metrics_list)), stringsAsFactors = FALSE)
performance_metrics <- rbind(performance_metrics, metrics_df_row)
  }
}
```
...
performance_summary <- performance_metrics %>%
  group_by(Missingness_Type, Method) %>%
  summarise(
...
# ARI Comparison
ggplot(performance_summary, aes(x = Missingness_Type, y = Mean_ARI, fill = Method)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean_ARI - SD_ARI, ymax = Mean_ARI + SD_ARI), width = .2, position = position_dodge(.9)) +
  labs(title = "Average ARI Comparison", x = "Missingness Type", y = "Average ARI") +
  theme_minimal()

# NMI Comparison
ggplot(performance_summary, aes(x = Missingness_Type, y = Mean_NMI, fill = Method)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean_NMI - SD_NMI, ymax = Mean_NMI + SD_NMI), width = .2, position = position_dodge(.9)) +
  labs(title = "Average NMI Comparison", x = "Missingness Type", y = "Average NMI") +
  theme_minimal()

# Silhouette Score Comparison
ggplot(performance_summary, aes(x = Missingness_Type, y = Mean_Silhouette, fill = Method)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean_Silhouette - SD_Silhouette, ymax = Mean_Silhouette + SD_Silhouette), width = .2, position = position_dodge(.9)) +
  labs(title = "Average Silhouette Score Comparison", x = "Missingness Type", y = "Average Silhouette Score") +
  theme_minimal()
```
